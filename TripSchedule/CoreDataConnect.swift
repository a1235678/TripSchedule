    ...
    func insert(_ myEntityName:String, attributeInfo:[String:Any]) -> Bool {
        ...
        for (key,value) in attributeInfo {
            ...
            if t == .integer16AttributeType || t == .integer32AttributeType || t == .integer64AttributeType {
                ...
                insetData.setValue(Int(newValue), forKey: key)
            } else if t == .doubleAttributeType || t == .floatAttributeType {
                let newValue = value as! Double
                insetData.setValue(Double(newValue), forKey: key)
            } else if t == .booleanAttributeType {
                let newValue = value as! String
                insetData.setValue((newValue == "true" ? true : false), forKey: key)
            } else if t == .dateAttributeType {
                let newValue = value as! Date
                insetData.setValue(newValue, forKey: key)
            } else {
                let newValue = value as! String
                insetData.setValue(newValue, forKey: key)
            }
